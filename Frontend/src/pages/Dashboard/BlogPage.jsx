...
export default function BlogPage() {
  ...
  const { user } = useAuth();
  const authorName = user?.name || (user?.email ? user.email.split('@')[0] : '—');
  ...
  const handleSave = async () => {
    ...
    if (!form.img)              missing.push('Cover image');
    if (!form.shortDescription) missing.push('Short description');
    if (!form.description)      missing.push('Full content');
    if (missing.length) {
          ...
          <table>
            <thead>
              <tr>
                ...
                <th className="hide-md">AUTHOR</th>
                <th className="hide-lg">EXCERPT</th>
                <th className="hide-md">DATE</th>
                <th className="td-sticky-right" style={{ textAlign: 'right' }}>ACTIONS</th>
              </tr>
            </thead>
            <tbody>
              {rows.map(row => (
                <tr key={row._id}>
                  ...
                  <td className="hide-md"><span className="badge badge-gold">{row.category}</span></td>
                  <td className="hide-md" style={{ fontSize: 12, color: 'var(--muted2)', whiteSpace: 'nowrap' }}>{authorName}</td>
                  <td className="hide-lg"><span className="truncate">{row.shortDescription}</span></td>
                  <td className="hide-md" style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--muted)', whiteSpace: 'nowrap' }}>
                    {row.createdAt ? new Date(row.createdAt).toLocaleDateString('en-GB', { day:'2-digit', month:'short', year:'2-digit' }) : '—'}
                  </td>
                  <td className="td-actions td-sticky-right">
