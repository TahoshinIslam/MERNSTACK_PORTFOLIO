            ...
            {filtered.map((item, i) => {
              ...
              return (
                <FadeIn key={item._id} delay={i * 0.06}>
                  <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden', transition: 'border-color 0.25s, transform 0.25s, box-shadow 0.25s', cursor: 'pointer', height: '100%', display: 'flex', flexDirection: 'column' }}
                  ...
                  >
                    ...
                    <div style={{ height: 200, background: `linear-gradient(135deg, ${color}18, ${color}08)`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
                      {item.img ? (
                        <img src={resolveImg(item.img)} alt={item.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => e.target.style.display='none'} />
                      ) : (
                      ...
                      {/* Category badge — uses live category, not a hardcoded list */}
                    ...
                    <div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 12, flex: 1 }}>
                      ...
                      {galleryCount > 0 && (
                        <div style={{ display: 'flex', gap: 6 }}>
                          {item.images.slice(0, 4).map((g, idx) => (
                            <div key={idx} style={{ width: 36, height: 36, borderRadius: 6, overflow: 'hidden', border: '1px solid var(--border)' }}>
